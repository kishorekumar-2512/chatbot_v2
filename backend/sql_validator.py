...
def extract_sql(text: str) -> str:
    ...
    if m and m.group(1).strip():
        ...
        m = re.search(r"((?:SELECT|WITH)\b.+?\bFROM\b.+?(?:;|$))", text, re.DOTALL | re.IGNORECASE)
        if m:
            sql = m.group(1).strip()
        else:
            sql = text.strip()
    # Strip any lingering leading/trailing markdown fences
...
def validate_sql(sql: str, known_tables: set, known_columns: dict) -> list[str]:
    ...
    if not re.search(r"\bFROM\b", sql, re.IGNORECASE):
        errors.append("Query must contain a FROM clause — conversational text is not valid SQL.")
        return errors
...
def validate_org_security(sql: str, org_id: str, known_columns: dict) -> list[str]:
    """
    Validates that the SQL query strictly contains the organization filter (zecure_org_id = org_id).
    Returns a list of security errors (empty = pass).
    """
    if not org_id:
        return []
    ...
    # Check that zecure_org_id = org_id is present in the SQL query
    pattern_eq = rf"\bzecure_org_id\s*=\s*['\"]?{re.escape(str(org_id))}['\"]?\b"
    pattern_in = rf"\bzecure_org_id\s+in\s*\(\s*['\"]?{re.escape(str(org_id))}['\"]?\s*\)"
    
    if not (re.search(pattern_eq, sql, re.IGNORECASE) or re.search(pattern_in, sql, re.IGNORECASE)):
        errors.append(
            f"Security violation: Query must strictly include the organization filter (zecure_org_id = {org_id})."
        )
